...
def xmltoExcel(select_folder, select_save_location):
    class ReadXML:
    ...
    # Extração de DADOS
        def extract_identificacao_nfe(self, root, nsNfe):
            ...
            return {
                    "Arquivo":chave,"cUF":cUF,"versao_XML":versao_XML,"cNF":cNF,"natOp":natOp,"indPag":indPag,"mod":mod,"serie":serie,
                    "nNF":nNF,"dhEmi":dhEmi,"dhSaiEnt":dhSaiEnt,"hSaiEnt":hSaiEnt,"tpNF":tpNF,"idDest":idDest,
                    "cMunFG":idcMunFG,"tpImp":tpImp,"tpEmis":tpEmis,"cDV":cDV,"tpAmb":tpAmb,"finNFe":finNFe,
                    "indFinal":indFinal,"indPres":indPres,"indIntermed":indIntermed,"procEmi":procEmi,"verProc":verProc,
                    "dhCont":dhCont,"xJust":xJust,"NFREF_refNFe":nFREF_refNFe,"REFNF_cUF":rEFNF_cUF,"REFNF_AAMM":rEFNF_AAMM,
                    "REFNF_CNPJ":rEFNF_CNPJ,"REFNF_mod":rEFNF_mod,"REFNF_serie":rEFNF_serie,"REFNF_nNF":rEFNF_nNF,
                    "REFNFP_cUF":rEFNFP_cUF,"REFNFP_AAMM":rEFNFP_AAMM,"REFNFP_CNPJ":rEFNFP_CNPJ,"REFNFP_CPF":rEFNFP_CPF,
                    "REFNFP_IE":rEFNFP_IE,"REFNFP_mod":rEFNFP_mod,"REFNFP_serie":rEFNFP_serie,"REFNFP_nNF":rEFNFP_nNF,
                    "refCTe":refCTe,"REFECF_mod":eFECF_mod,"REFECF_nECF":rREFECF_nECF,"REFECF_nCOO":rEFECF_nCOO
        }
    # Emitente (Extraindo informações de Emitente)
        def extract_emitente_data (self, root, nsNfe):
            chave =           self.check_none(root.find("./ns:protNFe/ns:infProt/ns:chNFe", nsNfe))
            ...
            return {
                    "Arquivo": chave,"idnNF": idnNF,"CNPJ": emit_CNPJ, "CPF": emit_CPF, "xNome": emit_xNome, "xFant": emit_xFant,
    ...
    # TOTAL IMPOSTOS (Extraindo informações do 'ICMSTot')
        def extract_total_data (self, root, nsNfe):
            chave =           self.check_none(root.find("./ns:protNFe/ns:infProt/ns:chNFe", nsNfe))
            idnNF =           self.check_none(root.find("./ns:NFe/ns:infNFe/ns:ide/ns:nNF", nsNfe))
            ...
            return{ 
                    "Arquivo": chave,"idnNF": idnNF, "vBC": totalvBC, "vICMS": vICMS, "vICMSDeson": vICMSDeson, "vFCPUFDest": vFCPUFDest,
    ...
    # TRANSPORTE 
        def extract_transportadora_data(self, root, nsNfe):
            volDados = []
            chave = self.check_none(root.find("./ns:protNFe/ns:infProt/ns:chNFe", nsNfe))
            idnNF = self.check_none(root.find("./ns:NFe/ns:infNFe/ns:ide/ns:nNF", nsNfe))
            modFrete = self.check_none(root.find("./ns:NFe/ns:infNFe/ns:transp/ns:modFrete", nsNfe))

            # Verifica se a transportadora existe no XML
            transportadora_elem = root.find("./ns:NFe/ns:infNFe/ns:transp/ns:transporta", nsNfe)
            if transportadora_elem is not None:
                transpcNPJ = self.check_none(transportadora_elem.find("./ns:CNPJ", nsNfe))
                transpcPF = self.check_none(transportadora_elem.find("./ns:CPF", nsNfe))
                xNome = self.check_none(transportadora_elem.find("./ns:xNome", nsNfe))
                transpIE = self.check_none(transportadora_elem.find("./ns:IE", nsNfe))
                xEnder = self.check_none(transportadora_elem.find("./ns:xEnder", nsNfe))
                xMun = self.check_none(transportadora_elem.find("./ns:xMun", nsNfe))
                transpUF = self.check_none(transportadora_elem.find("./ns:UF", nsNfe))
            else:
                transpcNPJ = transpcPF = xNome = transpIE = xEnder = xMun = transpUF = ""
            # Variáveis placeholders (não encontradas no XML)
            vServ = " "
            vBCRet = " "
            pICMSRet = " "
            vICMSRet = " "
            ...
            for vol in root.findall("./ns:NFe/ns:infNFe/ns:transp/ns:vol", nsNfe):
                ...
                volData = {
                    "Arquivo": chave,"idnNF": idnNF,"modFrete": modFrete if first_vol else "","CNPJ": transpcNPJ if first_vol else "",
                    "CPF": transpcPF if first_vol else "","xNome": xNome if first_vol else "","IE": transpIE if first_vol else "",
                    "xEnder": xEnder if first_vol else "","xMun": xMun if first_vol else "","UF": transpUF if first_vol else "",
                    "vServ": vServ if first_vol else "","vBCRet": vBCRet if first_vol else "","pICMSRet": pICMSRet if first_vol else "",
                    "vICMSRet": vICMSRet if first_vol else "","CFOP": cFOP if first_vol else "","cMunFG": cMunFG if first_vol else "",
                    "placa": placa if first_vol else "","Serv_uF": Serv_uF if first_vol else "","RNTC": rNTC if first_vol else "",
                    "destino_uF": destino_uF if first_vol else "","vagao": vagao if first_vol else "","balsa": balsa if first_vol else "",
                    "qVol": qVol,
                    "esp": esp,
                    "marca": marca,
                    "nVol": nVol,
    ...
    # Compras
        def extract_compras_data (self, root, nsNfe):
            ...
            return{
                "Arquivo": chave,"idnNF": idnNF, "xNEmp":xNEmp, "compras_xPed":compras_xPed, "xCont":xCont
            }
    # Resp. Tecnico (Exrtaçao de dados do Responsavel Tecnico)
        def extract_resp_tecnico_data (self, root, nsNfe):
            chave = self.check_none(root.find("./ns:protNFe/ns:infProt/ns:chNFe", nsNfe))
            idnNF = self.check_none(root.find("./ns:NFe/ns:infNFe/ns:ide/ns:nNF", nsNfe))    
            ...
            return {
                    "Arquivo": chave,"idnNF": idnNF, "CNPJ":tec_CNPJ, "xContato":tec_xContato, "email":tec_email, 
                    "fone":tec_fone, "idCSRT":tec_idCSRT, "hashCSRT":tec_hashCSRT
            }
    # Assinatura (Extração de dados da Assinatura)
        def extract_assinatura_data (self, root, nsNfe,nsSig):
            ...
            return {
                    "Arquivo": chave,"idnNF": idnNF, "DigestValue": digestValue, "SignatureValue": signatureValue, "X509Certificate":x509Certificate
            }
    # Protocolo (Extração de dados do protocolo)
        def extract_protocolo_data (self, root, nsNfe):
            chave = self.check_none(root.find("./ns:protNFe/ns:infProt/ns:chNFe", nsNfe))
            idnNF = self.check_none(root.find("./ns:NFe/ns:infNFe/ns:ide/ns:nNF", nsNfe))  
            ...
            return {
                    "Arquivo":chave,"idnNF":idnNF,"tpAmb":tpAmb,"verAplic":verAplic,"chNFe":chNFe,
                    "dhRecbto":dhRecbto,"nProt":nProt,"digVal":digVal,"cStat":cStat,"xMotivo":xMotivo
            }
    # Verifica se a variavel nao for None, a função retorna o atriburo .text se for None cria uma varavel vazia
        def check_none(self, var):
            return var.text if var is not None else ""
    # Este código seleciona uma pasta contendo arquivos XML e inicializa a leitura desses arquivos. Em seguida, extrai os dados de cada arquivo XML e os armazena em um dicionário. Após a extração, solicita ao usuário que selecione onde salvar o arquivo Excel resultante. Os dados extraídos são então escritos em um arquivo Excel com planilhas nomeadas conforme as chaves do dicionário. Caso ocorra qualquer exceção durante a execução, uma mensagem de erro é exibida.
    try:
        xml = ReadXML(select_folder)
        ...
        with pd.ExcelWriter(select_save_location) as writer:
